...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    const exerdetaildata = await fetchData(`https://fitness-app-backend5.onrender.com/api/exercise/${id}`);
    setexerdetail(exerdetaildata);

    const targetmuscle = await fetchData(`https://fitness-app-backend5.onrender.com/api/target/${exerdetaildata.target}`);
    settargetmuscleexer(targetmuscle);

    const equipmentdata = await fetchData(`https://fitness-app-backend5.onrender.com/api/equipment/${exerdetaildata.equipment}`);
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch(`https://fitness-app-backend5.onrender.com/api/youtube?query=${exerdetail.name}`);
