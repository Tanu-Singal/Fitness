...
const Detail = () => {
  ...
  useEffect(() => {
    ...
    const exerdetaildata = await fetchData(`http://localhost:5055/api/exercise/${id}`);
    setexerdetail(exerdetaildata);

    const targetmuscle = await fetchData(`http://localhost:5055/api/target/${exerdetaildata.target}`);
    settargetmuscleexer(targetmuscle);

    const equipmentdata = await fetchData(`http://localhost:5055/api/equipment/${exerdetaildata.equipment}`);
  ...
  const fetchVideos = async () => {
    try {
      const response = await fetch(`http://localhost:5055/api/youtube?query=${exerdetail.name}`);
