    ...
    const fetchCalories = async () => {
      try {
        const response = await fetch(
           `http://localhost:5055/api/calorie/user/${userId}?range=${viewMode}`
        );
 ...
 useEffect(() => {
    const fetchWorkoutDays = async () => {
      try {
        ...
        const res = await fetch(`http://localhost:5055/api/user/${userId}`); // Replace USER_ID with actual user ID or fetch from auth
