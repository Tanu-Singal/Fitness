...
const Login = ({ setWorkoutPlan }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    toast.warn("Please select at least one body focus area.");
    return;
  }
...
const handleNext = () => {
  if (step === 0) {
    if (!userData.name || !userData.age || !userData.weight || !gender || level === "") {
      toast.warn("Please fill all the user info fields.");
      return;
    }
    setStep(1);
  } else if (step === 1) {
    if (selectedDays.length === 0) {
      toast.warn("Please select at least one workout day.");
      return;
    }
    setStep(2);
  }
  ...
  {step > 0 && (
    <Button onClick={handlePrev} variant="outlined" color="secondary">
      Back
    </Button>
  )}

  {step < 2 && (
    <Button onClick={handleNext} variant="contained" sx={{ bgcolor: "#FF2625", color: "white" }}>
      Next
    </Button>
  )}
